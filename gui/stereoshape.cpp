...
void StereoShape::AskForData(Serializer *s){
...
void StereoShape::TreeScan(TSOCntx *cntx){ 
...
void StereoShape::Open(int slot){  
...
void StereoShape::LoadTxt(int slot){
...
void StereoShape::Draw(DrawCntx *cntx){
...
void StereoShape::reg_go(){

  if(w*h==0){    
    err_printf(("it seems image is not loaded. image size is (%ix%i)",w,h));
    return;
  }

  if(img[image_sel_s2].size()!=img[image_sel_s2].size()){
    err_printf(("it seems second slot is empty"
		" or size of the fist and second images are different"));
    return;
  }  

  if(img[image_sel_reg].size()!=w*h*4){
    img[image_sel_reg].resize(w*h*4);
  }
  StereoReg().run(&img[image_sel_reg][0],
		  &img[image_sel_s1][0],
		  &img[image_sel_s2][0], w, h);

  cache_slot=image_sel_off;
...
// event handler
//=================================================
StereoShapeEH::StereoShapeEH(StereoShape *v):
state_drag(-1),
ev(v)
{}

void StereoShapeEH::Handle(EventBall *eventball){
